...
function formatCommande(tabJson) {
    ...
    for (let i = 0; i < tabJson.length; i++) {
        ...
        strgCommande += "<br>Quantité: " + tabJson[i]["quantity"] + "<br>Prix Unitaire: " + tabJson[i]["price"] + "Є" +
            "<br>Prix total: " + (parseInt(tabJson[i]["price"]) * parseInt(tabJson[i]["quantity"]) + "Є") +
            "<br>----------";
...
function confChoix() {
    ...
    document.getElementById("messagePanier").innerHTML = "Combien de " + value[1] +
        " voulez-vous ajouter à votre commande?";
